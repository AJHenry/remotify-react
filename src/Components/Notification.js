...
import '../css/notification_stack.css';
  ...
  getIconUrlRetry() {
    ...
    var iconDownloaderHandle = setInterval(() => {
      ...
      var imageRef = storage
        ...
        .child(this.state.packageName);

      console.log(this.state.packageName);
      console.log(this.state.uid);

      imageRef
        .child(this.state.notificationId + '.png')
        .getDownloadURL()
        .then(url => {
    ...
    return (
      <div className="notification d-flex flex-row">
        <div className="titles-icon d-flex justify-content-start">
          {this.state.imgUrl ? (
            <div className="notification-icon d-flex justify-content-start">
              <img src={this.state.imgUrl} className="rounded float-left" />
            </div>
          ) : null}
        </div>
        <div className="notification-info">
          ...
          <div className="notification-body">{this.props.text}</div>
          <div className="notification-footer" />
        </div>
        <div className="notification-options d-flex flex-column justify-content-between">
          <ion-icon onClick={this.props.close} name="close" />
          {!this.props.bigText ? null : <ion-icon name="ios-arrow-down" />}
        </div>
      </div>
    );
  }
