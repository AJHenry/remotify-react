...
import NotificationStackHolder from './NotificationStackHolder';
...
class App extends Component {
  ...
  preparePackageBlock(packageName, notificationList) {
    ...
    const singleNotificationRef = db.ref(referenceString).child(uid).child(packageName);
    singleNotificationRef.on('child_added', (snapshot) => {

      var storage = firebase.storage();
      var imageRef = storage.ref('images').child(this.state.uid);
      imageRef.child(packageName + ".png").getDownloadURL().then((url) => {
    ...
    const notificationRef = db.ref(referenceString).child(uid).child(packageName);
    notificationRef.on('value', (snapshot) => {
      ...
      if (notificationList === null || notificationList.length === 0) {
        ...
        let generatedStack = this.generateNotificationStack(packageName, notificationList);
   ...
   */
  generateNotificationBlock(title, text, time, notificationId, packageName, isBigNotification, bigText, uid, hasLargeIcon) {
    return (
      <Notification
  ...
  removeNotification(id, packageName) {
    ...
    notificationRef.child(packageName).child(id).remove();
  }
